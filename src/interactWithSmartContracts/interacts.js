...
export const doPublicMint = async (
  globalWeb3,
  accountStr,
  numberOfNFTs,
  salePrice
  ...
  let saleContract;
  try {
    saleContract = new globalWeb3.eth.Contract(
      saleContractABI,
  ...
  try {
    ...
    let nativeValue = globalWeb3.utils.toWei(
      (Number(numberOfNFTs) * Number(salePrice)).toString(),
      "ether"
    );
    await funcTrx.send({
      from: accountStr,
...
export const isInALWL = async (globalWeb3, testingAddress) => {
  ...
  try {
    ...
    isIn = await saleContract.methods
      .isInALMerkleTree(testingAddress, al_proof)
      .call();
...
export const isInFLWL = async (globalWeb3, testingAddress) => {
  ...
  try {
    ...
    //////// To check if wallet is in whitelist on the contract side:
    isIn = await saleContract.methods
      .isInFreeMerkleTree(testingAddress, al_proof)
      .call();
